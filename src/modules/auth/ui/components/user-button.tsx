...
export const UserButton = ({ name, image, isInSession }: Props) => {
    ...
    return (
        <DropdownMenu modal={false}>
            ...
            <DropdownMenuContent className="mr-3 mt-4 w-96 border-dashed rounded-lg bg-Personient-to-b from-white/60 to-white/30 backdrop-blur-xl shadow-lg transition-all duration-300">
                <DropdownMenuLabel className="rounded-md mb-2 pt-2 px-2">
                    {name ? <>Hey, {name} ✨</> : <>Hey, Person 👀</>}
                </DropdownMenuLabel>
                <DropdownMenuSeparator />
                <DropdownMenuItem asChild className="p-2 rounded-md hover:bg-neutral-100 transition-colors font-semibold tracking-tight text-neutral-600">
                    <Link href="/studio">
                ...
                <DropdownMenuSeparator />
                <DropdownMenuItem onClick={handleAuth} className="p-2 rounded-md hover:bg-neutral-100 transition-colors font-semibold tracking-tight text-neutral-600">
                    {isInSession ? (
